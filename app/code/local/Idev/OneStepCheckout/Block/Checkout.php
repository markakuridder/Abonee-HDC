...
class Idev_OneStepCheckout_Block_Checkout extends Mage_Checkout_Block_Onepage_Abstract  {
    ...
    public function _handlePostData()
    {
        ...
        try {
            if(!empty($payment['method']) && $payment['method'] == 'checkmo'){

                $instance = Mage::helper('payment')->getMethodInstance('checkmo');
            ...
            // We will do the Credit Validation check here for the case when payment method
            // is Adyen. When Payment Method is Direct Debit the same check has already been
            // done on the module.
            
        	Mage::log($payment['method']);
        	$paymentError = false;
        	$paymentRedirect = false;
            if ($payment['method'] == 'adyen_hpp') {
            	
//             	$bankAccNr = $this->getOnepage()->getQuote()->getBillingAddress()->getBankAccount();
//             	if (empty($bankAccNr)) {
//             		$paymentError = true;
//             		Mage::getSingleton('popupmessage/message')->setMessageStaticBlockId('invalid_bank_acc_nr'); 
            		 
//             	}

//             	$validaterBankAccNr = new Crimsonwing_Validate_Bankaccount();
//             	Mage::log($bankAccNr);
//             	if(!$validaterBankAccNr->isValid($bankAccNr)) {
//             		Mage::log('Invalid Bank Acc Nr');
//             		$paymentError = true;
//             		Mage::getSingleton('popupmessage/message')->setMessageStaticBlockId('invalid_bank_acc_nr');            		
//             	}
            	...
            	// Else we will check Risk 
            	// This event is observed for Focum Credit checks 
            	// This event should have been fired after the first one.
	            //Mage::dispatchEvent('onestepcheckout_before_save_order_2', array('controller_action' => Mage::app()->getFrontController()));
        ...
        if(isset($result['error'])) {
        	...
        	Mage::dispatchEvent('onestepcheckout_before_save_order', array('controller_action' => Mage::app()->getFrontController()));
