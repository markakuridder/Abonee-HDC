...
//         $value = $read->query("select * from sales_flat_order_status_history where parent_id = '" . $this->getId() . "'");
...
//         Mage::log($adyenHtml);
        ...
        // Setting Bacnk Acount Nr. For When Payment is Adyen then we need to take
        // from Billing data.
         $paymentCode = $this->getPayment()->getMethodInstance()->getCode();

//          if ($paymentCode == 'adyen_hpp') {
//              $bankAccNr = $this->getBillingAddress()->getBankAccount();
//              $comment = $this->getOnestepcheckoutCustomercomment();
//              switch ($comment) {
//                  case 'ideal':
//                      $paymentMethod = 'iDeal';
//                      break;
...
//                      break;
//              }
//          } else { // From Direct Debit Payment Data.
//          }
             $bankAccNr = $this->getPayment()->getData('customer_bankaccount');
//              $paymentMethod = 'Bankoverschrijving'; //Direct debit @todo:

//         Mage::log($bankAccNr);

        // Shipping method based on Payment code

        $paymentCode = $this->getPayment()->getMethodInstance()->getCode();
//          $shippingText = 'PostNL pakketservice';


        // DOB
        $dobKey = 'customer_dob';
        $dob = $this->getData($dobKey);

        // Formatted Date
        $dobNl = Mage::app()->getLocale()->date(strtotime($dob), null, null, false)->toString('dd-MM-yyyy');
